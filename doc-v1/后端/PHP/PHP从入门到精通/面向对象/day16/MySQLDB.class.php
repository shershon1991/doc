...
class MySQLDB{
	...
	static function GetInstance($config){
		...
		if(!(self::$instance instanceof self)){	//这一行代替上一行的判断更常见
	...
	function GetOneRow($sql){
		...
		$result = $this->query($sql);
		//这里开始处理数据，以返回数组.此时$result是一个结果集（单行）
	...
	function GetRows($sql){
		...
		$result = $this->query($sql);
		//这里开始处理，以返回数组.此时$result是一个结果集（且是多行数据）
	...
	//这个方法用于执行任何sql语句，并进行错误处理，货返回执行结果；
	private function query($sql){
		$result = mysql_query($sql, $this->link);
		if($result === false){
			//对于任何sql语句,执行失败，则需要处理这种失败情况：
